/* 
INPUT = a big ol string consisting of a bunch of email messages, including body and metadata
RETURN = 
  1) number of email messages found in the string
  2) date range of email addresses

EMAIL MESSAGE
  - emails are separated by '##||##'
  - parts of each email message are separated by '#/#'.  Parts appear in sequence:
      1) Sender
      2) Subject
      3) Date
      4) Recipient
...
A
  - listOfDates = []
  - separate string into individual messages
  - for each message;
    - extract the date using regex
    - convert to a date object
    - append the date to listOfDates
  - sort listOfDates
  - output results
 */

function extractDate(emailString) {
  // Extracts the first date and returns as Date object
  let dateString = emailString.match(/^Date: ([0-9]{2}-){2}([0-9]{4})#\/#$/gm)
