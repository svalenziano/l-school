...
function calculator(str) {
  ...
  let stack = []
  let pointer = 0;
  let isNumeral = () => str[pointer] >= "0" && str[pointer] <= "9"
  while (pointer < str.length) {
    // find the next operator
    while (!["+", "-", "/", "*"].includes(str[pointer]) && pointer < str.length) {
      pointer += 1;
    }
    ...
    while (!isNumeral() && pointer < str.length) {
      pointer += 1
    }
    let number = 0;
    while (isNumeral() && pointer < str.length) {
      // console.log('str[pointer]', str[pointer])
  ...
  let result = stack.reduce((accum, ele) => accum + ele);
