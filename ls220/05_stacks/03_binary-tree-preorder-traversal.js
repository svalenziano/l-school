...
Rules:
  ...
  - The node is processed before either leg of the tree
  - Use a stack (not recursion)
...
Return = array of node values, omitting null values

Rule

v1 high level
  - result = []
  - Initialize stack with one element: root node
  - While stack is non-empty
    - pop node off stack
    - push node.val to `result`
    - if not null, push node.right to `stack`
    - if not null, push node.left to `stack`
