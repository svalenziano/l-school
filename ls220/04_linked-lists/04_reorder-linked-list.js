...
Side effects = mutate the nodes of the original LL

RULES = 
  - 1st node is odd, 2nd node is even, and so on
  - new LL = [odd nodes in original order, even nodes in orig. order]
...
function reorderOddEven(head) {
  ...
  let odd = head;
  let dummy = new ListNode('dummy', odd);
  let firstEven = odd.next;
  let even = firstEven;
