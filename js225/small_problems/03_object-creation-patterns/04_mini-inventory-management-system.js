...
const ItemManager = {
  ...
  inStock() {
    ...
    // console.log(i.map((item) => item.sku));
    return i;
  },
...
const ReportManager = {
  ...
  createReporter(sku) {
    // get reference to the object referenced by sku
    let item = this.items.returnItem(sku);
    if (item) return {
      itemInfo() {
...
//   ['deluxe stick', 'doggostuffs', 5],
//   ['bamboo', 'doggostuffs', 78],
...
// runTests(ReportManager.reportInStock, ReportManager, [[]])
// ItemManager.delete('DELDO');
// runTests(ReportManager.reportInStock, ReportManager, [[]])
...
ItemManager.update('SOCSP', { qty: 0 });
ItemManager.inStock();
...
ItemManager.update('KITCO', { qty: 10 });
kitchenPotReporter.itemInfo();
// logs
// skuCode: KITCO
// itemName: kitchen pot
